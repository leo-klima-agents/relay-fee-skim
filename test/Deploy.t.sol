...
/// @notice Exercises the deploy script against the CREATE2 deployer forge pre-deploys in every test.
contract DeployTest is Test {
    ...
    function test_run_deploysCanonicalConfiguration() public {
        assertEq(d.FEE_BPS(), 500);
        assertEq(d.FEE_SINK(), 0xf624f9Fe1D3165c5Ca32c7Fbdbf82f4a5b1D2d0e);

        address deployed = d.run();

        assertEq(deployed, d.predict(d.FEE_BPS(), d.FEE_SINK()));
        assertEq(RelayFeeSkim(deployed).FEE_BPS(), 500);
        assertEq(RelayFeeSkim(deployed).FEE_SINK(), 0xf624f9Fe1D3165c5Ca32c7Fbdbf82f4a5b1D2d0e);
    }

    function test_deploy_failsWhenConstructorReverts() public {
        vm.expectRevert(bytes("CREATE2 deploy failed"));
        d.deploy(0, SINK);
        vm.expectRevert(bytes("CREATE2 deploy failed"));
        d.deploy(500, address(0));
    }

    function test_deploy_failsWithoutDeployerOnChain() public {
